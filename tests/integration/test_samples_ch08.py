...
class TestChapter08:
    ...
    @pytest.mark.parametrize("v1", [10, 20])
    @pytest.mark.parametrize("v2", [1, 2])
    def test_simple_do(self, v0: int, v1: int, v2: int):
        target_file = SAMPLE_PROGRAM_DIR / TestChapter08.SUB_DIR / "simple_do.c"
        assert target_file.exists(), f"{target_file} not found"
    ...
    @pytest.mark.parametrize("v1", [10, 20])
    @pytest.mark.parametrize("v2", [1, 2])
    def test_simple_for(self, v0: int, v1: int, v2: int):
        target_file = SAMPLE_PROGRAM_DIR / TestChapter08.SUB_DIR / "simple_for.c"
        assert target_file.exists(), f"{target_file} not found"

        macros = [f"V0={v0}", f"V1={v1}", f"V2={v2}"]
        compile_run_check(target_file, macros=macros)

    @pytest.mark.parametrize("v0", [31, 39])
    def test_nested_loops(self, v0: int):
        target_file = SAMPLE_PROGRAM_DIR / TestChapter08.SUB_DIR / "nested_loops.c"
        assert target_file.exists(), f"{target_file} not found"
